...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        public CustomerController(ICustomerService customerService,
            ...
            IHttpContextAccessor currentContextAccessor)
        {
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    model.StoreAddress = customer.GetAttribute<string>(SystemCustomerAttributeNames.StoreAddress);
                    model.SubDomain = customer.GetAttribute<string>(SystemCustomerAttributeNames.SubDomain); 
                    model.StoreDescription = customer.GetAttribute<string>(SystemCustomerAttributeNames.StoreDescription);
                    model.BVN = customer.GetAttribute<string>(SystemCustomerAttributeNames.BVN);
                    model.CIVN = customer.GetAttribute<string>(SystemCustomerAttributeNames.CIVN);
                    ...
                    //model.PhoneNumber = customer.GetAttribute<string>(SystemCustomerAttributeNames.PhoneNumber);
                }
            }
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // checking if role is merchant
                if (newCustomerRoles.Any())
                {
                    if (newCustomerRoles.Any(m => m.SystemName.Trim().ToLower() == "merchant".Trim().ToLower()))
                    {
                        //also checking if customer was saved ok

                        if (customer.Id >= 1)
                        {
                            Store storemodel = new Store();
                            storemodel.Name = model.CIVN;
                            storemodel.CompanyName = model.Company;
                            storemodel.DisplayOrder = _storeService.GetAllStores().Count + 1;
                            //localhost/dg---local
                            //gd.loc.com
                            var hosturl = _currentContextAccessor.HttpContext.Request.Host + "/MerchantStore/Index?civn=" + model.CIVN.Trim().ToLower();
                            if (_currentEnvironment.IsDevelopment())
                            {
                                //localhost
                                storemodel.Url = hosturl;
                               // storemodel.Url = hosturl + "/" + model.CIVN.Trim().ToLower();
                            }
                            else
                            {
                                //live is a subdomain
                                storemodel.Url = hosturl;
                                //storemodel.Url = model.CIVN.Trim().ToLower() + hosturl;
                            }

                            //var activeUrl = storemodel.Url;
                      
                            //Response.Redirect("~/Customer/" + storemodel.Name);
                            _storeService.InsertStore(storemodel);
                ...
                _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.CIVN, model.CIVN);
                //_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PhoneNumber, model.PhoneNumber);
                _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.StoreAddress, model.StoreAddress);
                _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.StoreDescription, model.StoreDescription);
                _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.SubDomain, model.SubDomain);
                ...
                if (!string.IsNullOrEmpty(customer.Email))
                {
                    ...
                    {
                        var newsletterSubscription = _newsLetterSubscriptionService
                            .GetNewsLetterSubscriptionByEmailAndStoreId(customer.Email, stores.Id);
                        if (model.SelectedNewsletterSubscriptionStoreIds != null &&
                            model.SelectedNewsletterSubscriptionStoreIds.Contains(stores.Id))
                        {
                            //subscribed
                            if (newsletterSubscription == null)
                            {
