...
{
    public partial class CustomerController : BaseMerchantController
    {
        #region Fields
        ...
        protected virtual IList<RegisteredCustomerReportLineModel> GetReportRegisteredCustomersModel()
        {
            ...
            {
                Period = _localizationService.GetResource("Merchant.Customers.Reports.RegisteredCustomers.Fields.Period.7days"),
                Customers = _customerReportService.GetRegisteredCustomersReport(7)
            });

            report.Add(new RegisteredCustomerReportLineModel
            {
                Period = _localizationService.GetResource("Merchant.Customers.Reports.RegisteredCustomers.Fields.Period.14days"),
                Customers = _customerReportService.GetRegisteredCustomersReport(14)
            });
            report.Add(new RegisteredCustomerReportLineModel
            {
                Period = _localizationService.GetResource("Merchant.Customers.Reports.RegisteredCustomers.Fields.Period.month"),
                Customers = _customerReportService.GetRegisteredCustomersReport(30)
            });
            report.Add(new RegisteredCustomerReportLineModel
            {
                Period = _localizationService.GetResource("Merchant.Customers.Reports.RegisteredCustomers.Fields.Period.year"),
        ...
        protected virtual CustomerModel PrepareCustomerModelForList(Customer customer)
        {
            ...
            {
                Id = customer.Id,
                Email = customer.IsRegistered() ? customer.Email : _localizationService.GetResource("Merchant.Customers.Guest"),
        ...
        protected virtual string ValidateCustomerRoles(IList<CustomerRole> customerRoles)
        {
            ...
            if (isInGuestsRole && isInRegisteredRole)
                return _localizationService.GetResource("Merchant.Customers.Customers.GuestsAndRegisteredRolesError");
            if (!isInGuestsRole && !isInRegisteredRole)
                return _localizationService.GetResource("Merchant.Customers.Customers.AddCustomerToGuestsOrRegisteredRoleError");
        ...
        protected virtual void PrepareVendorsModel(CustomerModel model)
        {
            ...
            {
                Text = _localizationService.GetResource("Merchant.Customers.Customers.Fields.Vendor.None"),
                Value = "0"
            });
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    model.MerchantComment = customer.AdminComment;
                    ...
                    model.BVN = customer.GetAttribute<string>(SystemCustomerAttributeNames.BVN);
                }
            }
            ...
            {
                model.AvailableCountries.Add(new SelectListItem { Text = _localizationService.GetResource("Merchant.Address.SelectCountry"), Value = "0" });
                ...
                if (_customerSettings.StateProvinceEnabled)
                {
                    ...
                    if (states.Any())
                    {
                        model.AvailableStates.Add(new SelectListItem { Text = _localizationService.GetResource("Merchant.Address.SelectState"), Value = "0" });
                    ...
                    else
                    {
                        ...
                        {
                            Text = _localizationService.GetResource(anyCountrySelected ? "Merchant.Address.OtherNonUS" : "Merchant.Address.SelectState"),
                            Value = "0"
                        });
                    }
                }
            ...
            var MerchantRole = allRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.Registered);
            //precheck Registered Role as a default role while creating a new customer through Merchant
            if (customer == null && MerchantRole != null)
            {
                model.SelectedCustomerRoleIds.Add(MerchantRole.Id);
            }
            ...
            {
                model.AddRewardPoints.Message = _localizationService.GetResource("Merchant.Customers.Customers.SomeComment");
            ...
            //1. "Merchant approval" registration method
        ...
        protected virtual void PrepareAddressModel(CustomerAddressModel model, Address address, Customer customer, bool excludeProperties)
        {
            ...
            //countries
            model.Address.AvailableCountries.Add(new SelectListItem { Text = _localizationService.GetResource("Merchant.Address.SelectCountry"), Value = "0" });
            foreach (var c in _countryService.GetAllCountries(showHidden: true))
            ...
            if (states.Any())
            {
                ...
                model.Address.AvailableStates.Add(new SelectListItem { Text = _localizationService.GetResource("Merchant.Address.OtherNonUS"), Value = "0" });
            //customer attribute services
            model.Address.PrepareCustomAddressAttributes(address, _addressAttributeService, _addressAttributeParser);
        }

        private bool SecondMerchantAccountExists(Customer customer)
        {
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing)
        {
            ...
            {
                ModelState.AddModelError("", _localizationService.GetResource("Merchant.Customers.Customers.ValidEmailRequiredRegisteredRole"));
                ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.ValidEmailRequiredRegisteredRole"), false);
            }
            ...
            if (ModelState.IsValid)
            {
                var customer = new Customer
                {
                    ...
                    AdminComment = model.MerchantComment,
                ...
                {
                    //ensure that the current customer cannot add to "Merchantistrators" system role if he's not an Merchant himself
                ...
                //ensure that a customer with a vendor associated is not in "Merchantistrators" role
                //otherwise, he won't have access to other functionality in Merchant area
                if (customer.IsAdmin() && customer.VendorId > 0)
                {
                    customer.VendorId = 0;
                    _customerService.UpdateCustomer(customer);
                    ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.MerchantCouldNotbeVendor"));
                }
                ...
                if (customer.IsVendor() && customer.VendorId == 0)
                {
                    ...
                    _customerService.UpdateCustomer(customer);
                    ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.CannotBeInVendoRoleWithoutVendorAssociated"));
                }
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.Added"));
        ...
        public virtual IActionResult Edit(CustomerModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    customer.AdminComment = model.MerchantComment;
                    customer.IsTaxExempt = model.IsTaxExempt;

                    //prevent deactivation of the last active Merchantistrator
                    if (!customer.IsAdmin() || model.Active || SecondMerchantAccountExists(customer))
                        customer.Active = model.Active;
                    else
                        ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.MerchantAccountShouldExists.Deactivate"));
                    ...
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.FirstName, model.FirstName);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.LastName, model.LastName);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.BVN, model.BVN);
                    if (_customerSettings.DateOfBirthEnabled)
                        _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.DateOfBirth, model.DateOfBirth);
                    if (_customerSettings.CompanyEnabled)
                        _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Company, model.Company);
                    ...
                    foreach (var customerRole in allCustomerRoles)
                    {
                        //ensure that the current customer cannot add/remove to/from "Merchantistrators" system role
                        //if he's not an Merchant himself
                        if (customerRole.SystemName == SystemCustomerRoleNames.Administrators &&
                            !_workContext.CurrentCustomer.IsAdmin())
                            continue;
                        ...
                        {
                            //prevent attempts to delete the Merchantistrator role from the user, if the user is the last active Merchantistrator
                            if (customerRole.SystemName == SystemCustomerRoleNames.Administrators && !SecondMerchantAccountExists(customer))
                            {
                                ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.MerchantAccountShouldExists.DeleteRole"));
                                continue;
                            }
                    ...
                    //ensure that a customer with a vendor associated is not in "Merchantistrators" role
                    //otherwise, he won't have access to the other functionality in Merchant area
                    if (customer.IsAdmin() && customer.VendorId > 0)
                    {
                        customer.VendorId = 0;
                        _customerService.UpdateCustomer(customer);
                    ...
                    SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.Updated"));
                    if (continueEditing)
                    {
                        //selected tab
                        SaveSelectedTabName();
        ...
        public virtual IActionResult ChangePassword(CustomerModel model)
        {
            ...
            //ensure that the current customer cannot change passwords of "Merchantistrators" if he's not an Merchant himself
            if (customer.IsAdmin() && !_workContext.CurrentCustomer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.OnlyMerchantCanChangePassword"));
                return RedirectToAction("Edit", new { id = customer.Id });
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                var changePassResult = _customerRegistrationService.ChangePassword(changePassRequest);
                if (changePassResult.Success)
                    SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.PasswordChanged"));
                else
        ...
        public virtual IActionResult Delete(int id)
        {
            ...
            try
            {
                //prevent attempts to delete the user, if it is the last active Merchantistrator
                if (customer.IsAdmin() && !SecondMerchantAccountExists(customer))
                {
                    ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.MerchantAccountShouldExists.DeleteMerchantistrator"));
                    return RedirectToAction("Edit", new { id = customer.Id });
                }

                //ensure that the current customer cannot delete "Merchantistrators" if he's not an Merchant himself
                if (customer.IsAdmin() && !_workContext.CurrentCustomer.IsAdmin())
                {
                    ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.OnlyMerchantCanDeleteMerchant"));
                    return RedirectToAction("Edit", new { id = customer.Id });
                }
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.Deleted"));
                return RedirectToAction("List");
            }
            catch (Exception exc)
            {
        ...
        public virtual IActionResult Impersonate(int id)
        {
            ...
            //ensure that a non-Merchant user cannot impersonate as an Merchantistrator
            //otherwise, that user can simply impersonate as an Merchantistrator and gain additional Merchantistrative privileges
            if (!_workContext.CurrentCustomer.IsAdmin() && customer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Merchant.Customers.Customers.NonMerchantNotImpersonateAsMerchantError"));
        ...
        public virtual IActionResult SendWelcomeMessage(CustomerModel model)
        {
            ...
            SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.SendWelcomeMessage.Success"));
        ...
        public virtual IActionResult ReSendActivationMessage(CustomerModel model)
        {
            ...
            SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.ReSendActivationMessage.Success"));
        ...
        public virtual IActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.SendEmail.Queued"));
            }
            catch (Exception exc)
            {
                ErrorNotification(exc.Message);
        ...
        public virtual IActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.SendPM.Sent"));
            }
            catch (Exception exc)
            {
                ErrorNotification(exc.Message);
                ...
                Data = rewardPoints.Select(historyEntry =>
                {
                    ...
                    return new CustomerModel.RewardPointsHistoryModel
                    {
                        ...
                        PointsBalance = historyEntry.PointsBalance.HasValue ? historyEntry.PointsBalance.ToString() : 
                            string.Format(_localizationService.GetResource("Merchant.Customers.Customers.RewardPoints.ActivatedLater"), activatingDate),
        ...
        public virtual IActionResult RewardPointsHistoryAdd(CustomerModel.AddRewardPointsModel model)
        {
            ...
            if (model.Points == 0)
                return Json(new { Result = false, Error = _localizationService.GetResource("Merchant.Customers.Customers.RewardPoints.AddingZeroValueNotAllowed") });
        ...
        public virtual IActionResult AddressCreate(CustomerAddressModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.Addresses.Added"));
        ...
        public virtual IActionResult AddressEdit(CustomerAddressModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                SuccessNotification(_localizationService.GetResource("Merchant.Customers.Customers.Addresses.Updated"));
        ...
        public virtual IActionResult Reports()
        {
            ...
            model.BestCustomersByNumberOfOrders.AvailableOrderStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
            model.BestCustomersByNumberOfOrders.AvailablePaymentStatuses = PaymentStatus.Pending.ToSelectList(false).ToList();
            model.BestCustomersByNumberOfOrders.AvailablePaymentStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
            model.BestCustomersByNumberOfOrders.AvailableShippingStatuses = ShippingStatus.NotYetShipped.ToSelectList(false).ToList();
            model.BestCustomersByNumberOfOrders.AvailableShippingStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
            ...
            model.BestCustomersByOrderTotal.AvailableOrderStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
            model.BestCustomersByOrderTotal.AvailablePaymentStatuses = PaymentStatus.Pending.ToSelectList(false).ToList();
            model.BestCustomersByOrderTotal.AvailablePaymentStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
            model.BestCustomersByOrderTotal.AvailableShippingStatuses = ShippingStatus.NotYetShipped.ToSelectList(false).ToList();
            model.BestCustomersByOrderTotal.AvailableShippingStatuses.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Merchant.Common.All"), Value = "0" });
        ...
        public virtual IActionResult ReportBestCustomersByOrderTotalList(DataSourceRequest command, BestCustomersReportModel model)
        {
            ...
            var gridModel = new DataSourceResult
            {
                Data = items.Select(x =>
                {
                    ...
                    if (customer != null)
                    {
                        m.CustomerName = customer.IsRegistered() ? customer.Email : _localizationService.GetResource("Merchant.Customers.Guest");
                    }
                    return m;
                }),
