...
namespace Nop.Web.Areas.Merchant.Validators.Catalog
{
    public partial class ProductReviewValidator : BaseNopValidator<ProductReviewModel>
    {
        public ProductReviewValidator(ILocalizationService localizationService, IDbContext dbContext, IWorkContext workContext)
        {
            ...
            {
                RuleFor(x => x.Title).NotEmpty().WithMessage(localizationService.GetResource("Merchant.Catalog.ProductReviews.Fields.Title.Required"));
                RuleFor(x => x.ReviewText).NotEmpty().WithMessage(localizationService.GetResource("Merchant.Catalog.ProductReviews.Fields.ReviewText.Required"));
            }
