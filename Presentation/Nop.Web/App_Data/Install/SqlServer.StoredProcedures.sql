...
BEGIN
	...
	BEGIN
		SET @sql = @sql + '
		AND p.CustomerId = ' + CAST(@CustomerId AS nvarchar(max))
	END
...
CREATE PROCEDURE [dbo].[spGetAllMerchant]
AS
BEGIN
  SET NOCOUNT ON
 
  select c.*,ga.[Value] as CIVN
       from [CAPDAN].dbo.Customer c
      inner join  dbo.Customer_CustomerRole_Mapping crm on c.Id = crm.Customer_Id
	  left join [dbo].[GenericAttribute]  ga on c.Id=ga.EntityId
	  where ga.KeyGroup='Customer' and ga.[Key]='CIVN'
      --inner join [CAPDAN].dbo.CustomerRole cr on c.Id = crm.CustomerRole_Id
      and crm.CustomerRole_Id = 6;
END
GO


CREATE PROCEDURE [dbo].[spGetAllProductsByCustomerID]
  @CustomerID INT
...
--inner join dbo.Customer_CustomerRole_Mapping CCM on C.Id = CCM.Customer_Id
--inner join dbo.CustomerRole CR on CCM.CustomerRole_Id = CR.Id 
--where  CR.Id = 6 AND CCM.Customer_Id = @CustomerID ;
...
CREATE PROCEDURE [dbo].[GetMerchantCustomerByCIVN]
(
	@civn nvarchar(250)
)
AS
BEGIN
	...
	SELECT TOP (1) c.*
  FROM [dbo].[Customer] c
  inner join [dbo].[GenericAttribute] g on c.Id=g.EntityId
  where g.KeyGroup='Customer' and g.[Key]='CIVN' and LOWER(g.[Value]) =LOWER(@civn)

END
GO


CREATE PROCEDURE [dbo].[spGetAllProductsByCIVN]
  @civn varchar(255)
...
inner join dbo.Customer c on P.CustomerId = c.Id
left join [dbo].[GenericAttribute]  ga on c.Id=ga.EntityId
where ga.KeyGroup='Customer' and ga.[Key]='CIVN' and ga.[Value]= @civn
